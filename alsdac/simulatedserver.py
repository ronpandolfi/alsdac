...
BUFSIZE = 16384
fakemotor = {'pos': 10., 'precision': 1e-4, 'limits': (-1., 1.)}
    ...
    with server_sock:
        ...
        try:
            while True:
                ...
                # print(command)
                value = data.split(b'(')[1][:-1]

                msg = 'Error: Unknown Command'
                if command == b'GetMotorPos':
                    msg = bytes(str(fakemotor['pos']), 'utf-8')
                elif command == b'MoveMotor':
                    fakemotor['pos'] = float(value.split(b',')[1])
                    msg = bytes(True)
                elif command == b'GetSoftLimits':
                    msg = bytes(' '.join(map(str, fakemotor['limits'])), 'utf-8')
                print("echo_server {}: sending data {!r}".format(ident, msg))
                await server_sock.sendall(msg)
